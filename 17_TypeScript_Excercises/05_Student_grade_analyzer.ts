...
function analyzeGrades(student: StudentInfo, minimumPasGrade: number)
  : GradeAnalysisReport {
  ...
  const passed = averageGrade >= minimumPasGrade;

  // Determine the highest grade
  const highestGrade = Math.max(...student.grades);

  let summary = passed ?
    `${student.name}'s average grade is ${averageGrade.toFixed(2)}. Excellent! The student passed with a highest grade of ${highestGrade.toFixed(2)}.` :
    `${student.name}'s average grade is ${averageGrade.toFixed(2)}. Unfortunately, the student did not meet the minimum grade of ${minimumPasGrade}.`;
